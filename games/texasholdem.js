...
/* Opponent tags — above the oval, horizontal pill format:
   [dot] Name   chips   action */
...
/* Showdown: only opponent cards — my big cards stay below */
...
/* Right column: chips (3-col grid) + action buttons (3-col grid, aligned) */
...
function renderTexas(){
    ...
    /* Table interior: always show community cards, showdown strip below if applicable */
    const showdownInner=(function(){
        if(!isShowdown)return"";
        const oppHtml=buildShowdownHtml();
        return oppHtml
            ?"<div class=\"th-sub-title th-sub-mt\">Showdown</div><div class=\"th-show-row\">"+oppHtml+"</div>"
            :"";
    })();
    ...
    /* ── CSS ── */
    const css=[
        /* Wrapper — extra top padding to push content down */
        ".th-wrap{height:100%;overflow:auto;padding:14px 8px 82px;box-sizing:border-box;font-family:Arial,sans-serif;color:#e2f0d9;background:radial-gradient(ellipse at 50% 25%,#14302a 0%,#07150e 100%);}",

        /* Two thin blind pills */
        ".th-top{display:flex;gap:8px;max-width:540px;margin:0 auto 10px;}",
        ".th-bpill{flex:1;display:flex;align-items:center;gap:6px;background:rgba(0,0,0,.4);border:1px solid rgba(255,255,255,.13);border-radius:20px;padding:5px 12px;font-size:12px;font-weight:700;color:#e2f0d9;}",
        ".th-bpill b{color:#ffd700;margin-left:auto;font-size:13px;}",
        ".th-bdot{width:9px;height:9px;border-radius:50%;flex-shrink:0;}",
        ".th-bdot-bb{background:#e53935;box-shadow:0 0 4px rgba(229,57,53,.5);}",
        ".th-bdot-sb{background:#43a047;box-shadow:0 0 4px rgba(67,160,71,.5);}",

        /* Status */
        ".th-message{text-align:center;color:#ffd700;font-weight:900;font-size:14px;line-height:1.25;max-width:540px;margin:0 auto 3px;text-shadow:0 1px 5px rgba(0,0,0,.6);}",
        ".th-last{text-align:center;color:#a8e6cf;font-size:12px;font-weight:700;max-width:540px;margin:0 auto 4px;}",

        /* Opponent tags — above the oval */
        ".th-opp-wrap{display:flex;flex-wrap:wrap;gap:6px;justify-content:center;max-width:540px;margin:0 auto 6px;}",
        ".th-opp{display:inline-flex;align-items:center;gap:5px;background:rgba(0,0,0,.42);border:1px solid rgba(255,255,255,.1);border-radius:16px;padding:3px 10px;font-size:10px;color:#e2f0d9;white-space:nowrap;}",
        ".th-opp.active{border-color:#ff4444;box-shadow:0 0 0 1px #ff4444;}",
        ".th-opp.folded{opacity:.38;}",
        ".th-odot{width:7px;height:7px;border-radius:50%;flex-shrink:0;}",
        ".th-odot-sb{background:#43a047;}",
        ".th-odot-bb{background:#e53935;}",
        ".th-odot-n{background:#555;}",
        ".th-on{font-weight:900;font-size:10px;}",
        ".th-oc{color:rgba(226,240,217,.65);font-weight:700;}",
        ".th-oa{color:#ff8a80;font-weight:700;}",
        ".th-opp.folded .th-oa{color:#555;}",

        /* Poker table oval — pushed down via opp-wrap margin above */
        ".th-table{max-width:540px;margin:0 auto 10px;background:radial-gradient(ellipse 92% 80% at 50% 50%,#2e7d52 0%,#1d5c39 50%,#113d26 100%);border-radius:100px;border:15px solid #5c3319;box-shadow:inset 0 3px 10px rgba(255,255,255,.08),inset 0 -4px 14px rgba(0,0,0,.55),0 0 0 3px #3c2110,0 0 0 5px #8a5030,0 0 0 7px #3c2110,0 10px 36px rgba(0,0,0,.75);padding:14px 14px 14px;position:relative;}",
        ...
        ".th-sub-title{text-align:center;color:rgba(255,255,255,.4);font-size:9px;text-transform:uppercase;letter-spacing:1.2px;font-weight:700;margin:0 0 5px;}",
        ".th-sub-mt{margin-top:10px;}",
        ".th-community{display:flex;gap:5px;justify-content:center;}",
        ...
        /* Showdown strip — opponent cards only */
        ".th-show-row{display:flex;flex-wrap:wrap;gap:7px;justify-content:center;}",
        ".th-show-p{background:rgba(8,22,14,.95);border:1.5px solid rgba(255,255,255,.12);border-radius:9px;padding:5px 8px;text-align:center;}",
        ".th-show-p.winner{border-color:#ffd700;box-shadow:0 0 0 1.5px #ffd700,0 0 10px rgba(255,215,0,.35);}",
        ...
        ".th-card-sm{width:34px;height:48px;}",
        ...
        ".th-myinfo{display:flex;align-items:center;gap:6px;margin-bottom:8px;}",
        ".th-my-dot{width:9px;height:9px;border-radius:50%;flex-shrink:0;}",
        ".th-my-dot-sb{background:#43a047;box-shadow:0 0 4px rgba(67,160,71,.5);}",
        ".th-my-dot-bb{background:#e53935;box-shadow:0 0 4px rgba(229,57,53,.5);}",
        ...
        /* Right column */
        ".th-myright{flex:1;display:flex;flex-direction:column;gap:5px;min-width:0;}",
        ".th-waiting{font-size:12px;color:rgba(226,240,217,.45);font-weight:700;padding-top:8px;line-height:1.5;}",
        ".th-pend-lbl{font-size:12px;font-weight:700;color:#ffd700;line-height:1.4;display:flex;align-items:center;gap:6px;}",
        ".th-pend-lbl b{font-size:14px;}",
        ".th-pltip{font-size:11px;color:rgba(226,240,217,.4);font-weight:400;}",

        /* Chips — 3-column grid matching buttons below */
        ".th-chips-row{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;width:100%;}",

        /* Poker chip — 34px, rings sized to not overlap at 8px grid gap */
        ".th-chip{width:34px;height:34px;border-radius:50%;border:none;cursor:pointer;justify-self:center;display:flex;align-items:center;justify-content:center;font-weight:900;font-size:10px;-webkit-tap-highlight-color:transparent;transition:transform .08s;}",
        ".th-chip:active{transform:scale(.86);}",
        ".th-chip span{pointer-events:none;}",
        ".th-cw{background:#f0ead8;color:#2a2a2a;box-shadow:0 0 0 1.5px #b8ae98,0 0 0 3px #f0ead8,0 0 0 5px #b8ae98,0 2px 5px rgba(0,0,0,.5);}",
        ".th-cr{background:#c0392b;color:#fff;box-shadow:0 0 0 1.5px #8b1a10,0 0 0 3px #c0392b,0 0 0 5px #8b1a10,0 2px 5px rgba(0,0,0,.5);}",
        ".th-cb{background:#1a1a2e;color:#ffd700;box-shadow:0 0 0 1.5px #c9a84c,0 0 0 3px #1a1a2e,0 0 0 5px #c9a84c,0 2px 5px rgba(0,0,0,.5);}",

        ".th-chip-clear{background:rgba(255,255,255,.12);border:1.5px solid rgba(255,255,255,.25);border-radius:50%;width:22px;height:22px;color:#fff;font-size:11px;cursor:pointer;display:flex;align-items:center;justify-content:center;-webkit-tap-highlight-color:transparent;flex-shrink:0;}",

        /* Action buttons — same 3-col grid as chips for alignment */
        ".th-act-btns{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;width:100%;}",
        ".th-act-btns button{border:none;border-radius:10px;padding:8px 2px;font-size:11px;font-weight:900;cursor:pointer;-webkit-tap-highlight-color:transparent;box-shadow:0 2px 5px rgba(0,0,0,.4);width:100%;}",
        ...
        /* Rules */
        ".th-rules{font-size:10px;text-align:center;color:rgba(226,240,217,.32);max-width:540px;margin:4px auto 0;line-height:1.4;}",

        /* Responsive */
        "@media(max-width:390px){",
            ".th-wrap{padding:10px 6px 80px;}",
            ...
            ".th-chip{width:30px;height:30px;font-size:9px;}",
        "}"
    ].join("");
    ...
    el.innerHTML=[
        ...
        /* Poker table oval — no seat tags inside */
        "<div class=\"th-table\">",
            tableMiddle,
        "</div>",
