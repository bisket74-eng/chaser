...
// Flashes the header briefly when a new entry lands, and schedules a
// re-render for when this entry's pop window expires so the header reverts
// to the live "whose turn" status even if nothing else changes state in the
...
function buildSvgBoardHtml() {
    ...
    (st.board.ports || []).forEach(port => {
        ...
        const clampedLx = Math.max(viewMinX + badgeRadius + safeMargin, Math.min(viewMaxX - badgeRadius - safeMargin, rawLabelPoint.x));
        const clampedLy = Math.max(viewMinY + badgeRadius + safeMargin, Math.min(viewMaxY - badgeRadius - safeMargin, rawLabelPoint.y));
        ...
        const fontSize = port.type === "3:1" ? 26 : 24;
        ...
        portsHtml += `
            <g pointer-events="none">
                ...
                <circle cx="${clampedLx}" cy="${clampedLy}" r="${badgeRadius}" fill="#eaf4df" stroke="#1e4620" stroke-width="2.5"/>
                <text x="${clampedLx}" y="${clampedLy + fontSize * 0.36}" text-anchor="middle" font-family="Arial" font-size="${fontSize}" font-weight="900" fill="#1e4620">${label}</text>
            </g>
        `;
    });
...
function buildSettlersLogHeaderHtml() {
    ...
    const topLogEntry = log.length ? log[log.length - 1] : null;
    ...
    const isFresh = !transientActive && topLogEntry && typeof topLogEntry.createdAt === "number" && (Date.now() - topLogEntry.createdAt) < LOG_POP_DURATION_MS;
    ...
    if (transientActive) {
        ...
        headerText = topLogEntry.text;
        headerDot = logDotHtml(topLogEntry.dotPlayerId);
    } else {
