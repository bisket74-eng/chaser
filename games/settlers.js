...
/* SETTLERS PATCH: shuffle/start, road outline, dice bag, robber */
(function(){
"use strict";
if(window.__settlersShuffleStartDiceRobberPatch)return;
window.__settlersShuffleStartDiceRobberPatch=true;
const RT=["brick","wheat","sheep","wood","ore"],HS=48,HW=Math.sqrt(3)*HS;
function me(){if(typeof window.myId==="function")return window.myId();if(typeof window.myId==="string")return window.myId;return localStorage.getItem("rider_id")||"local-player";}
function host(){return !window.chaserGame||!window.chaserGame.hostId||window.chaserGame.hostId===me();}
function gid(){return window.chaserGame&&window.chaserGame.activeGameId?window.chaserGame.activeGameId:null;}
function s(){return window.settlersState;}
function cur(){const x=s();if(!x||!Array.isArray(x.players)||!x.players.length)return{id:me(),name:"Player",isComputer:false};if(x.setup&&x.setup.active&&Array.isArray(x.setup.order)){const seat=x.setup.order[x.setup.stepIndex]||0;return x.players[seat]||x.players[0];}return x.players[x.turnIndex%x.players.length]||x.players[0];}
function sync(){if(typeof channel!=="undefined"&&channel&&typeof channel.send==="function"&&s())channel.send({type:"broadcast",event:"settlers-sync-state",payload:{state:s(),roomGameId:gid()}});}
function rerender(doSync){const x=s();if(!x)return;if(typeof window.handleIncomingSettlersSync==="function")window.handleIncomingSettlersSync({state:x,roomGameId:gid()});else{const m=document.getElementById("set-msg");if(m&&x.message)m.innerText=x.message;}setTimeout(drawExtras,35);if(doSync!==false)sync();}
function rnd(max){if(max<=0)return 0;if(window.crypto&&window.crypto.getRandomValues){const a=new Uint32Array(1),lim=Math.floor(0x100000000/max)*max;let n;do{window.crypto.getRandomValues(a);n=a[0];}while(n>=lim);return n%max;}return Math.floor(Math.random()*max);}
function sh(a){a=a.slice();for(let i=a.length-1;i>0;i--){const j=rnd(i+1),t=a[i];a[i]=a[j];a[j]=t;}return a;}
function diceBag(){const b=[];for(let d1=1;d1<=6;d1++)for(let d2=1;d2<=6;d2++)b.push({d1,d2,total:d1+d2});return sh(b);}
function rollFromBag(x){if(!Array.isArray(x._diceBag)||!x._diceBag.length)x._diceBag=diceBag();let r=x._diceBag.pop();x._diceLastTotals=Array.isArray(x._diceLastTotals)?x._diceLastTotals:[];const last=x._diceLastTotals.slice(-2);if(last.length===2&&last[0]===r.total&&last[1]===r.total&&x._diceBag.length){const k=x._diceBag.findIndex(v=>v.total!==r.total);if(k>=0){const q=x._diceBag.splice(k,1)[0];x._diceBag.push(r);r=q;}}x._diceLastTotals.push(r.total);if(x._diceLastTotals.length>8)x._diceLastTotals.shift();return r;}
function empty(){return{brick:0,wheat:0,sheep:0,wood:0,ore:0};}
function cards(pid){const x=s();if(!x)return empty();x.resources=x.resources||{};x.resources[pid]=x.resources[pid]||empty();RT.forEach(t=>{if(typeof x.resources[pid][t]!=="number")x.resources[pid][t]=0;});return x.resources[pid];}
function corners(h){return[{x:h.x,y:h.y-HS},{x:h.x+HW/2,y:h.y-HS/2},{x:h.x+HW/2,y:h.y+HS/2},{x:h.x,y:h.y+HS},{x:h.x-HW/2,y:h.y+HS/2},{x:h.x-HW/2,y:h.y-HS/2}];}
function pm(a,b){return Math.round(a.x)===Math.round(b.x)&&Math.round(a.y)===Math.round(b.y);}
function touches(piece,h){return corners(h).some(c=>pm(c,piece));}
function desert(x){if(!x||!x.board||!Array.isArray(x.board.hexes))return 0;const i=x.board.hexes.findIndex(h=>h.resource==="desert");return i>=0?i:0;}
function ensureRobber(x){if(!x||!x.board||!Array.isArray(x.board.hexes))return;if(!x.robber||typeof x.robber.hexIndex!=="number"||!x.board.hexes[x.robber.hexIndex])x.robber={hexIndex:desert(x)};}
function robberToDesert(x){if(!x||!x.board||!Array.isArray(x.board.hexes))return;x.robber={hexIndex:desert(x)};x.awaitingRobberMove=null;}
function gains(total){const x=s(),g={};if(!x||!x.board||!Array.isArray(x.board.hexes)||!x.pieces)return g;ensureRobber(x);function add(pid,res,n){if(!pid||!res||res==="desert"||!n)return;if(!g[pid])g[pid]=empty();g[pid][res]+=n;}x.board.hexes.forEach((h,i)=>{if(i===(x.robber?x.robber.hexIndex:-1))return;if(h.token!==total||h.resource==="desert")return;(x.pieces.settlements||[]).forEach(p=>{if(touches(p,h))add(p.owner,h.resource,1);});(x.pieces.cities||[]).forEach(p=>{if(touches(p,h))add(p.owner,h.resource,2);});});return g;}
function applyG(g){Object.keys(g||{}).forEach(pid=>{const c=cards(pid);RT.forEach(t=>c[t]+=g[pid][t]||0);});}
function adj(a,b){const d=Math.hypot(a.x-b.x,a.y-b.y);return d<88&&d>70;}
function boardScore(hexes){let sc=0;for(let i=0;i<hexes.length;i++)for(let j=i+1;j<hexes.length;j++){if(!adj(hexes[i],hexes[j]))continue;const a=hexes[i],b=hexes[j];if(a.resource&&b.resource&&a.resource===b.resource&&a.resource!=="desert")sc-=150;if(a.token&&b.token&&a.token===b.token)sc-=220;if((a.token===6||a.token===8)&&(b.token===6||b.token===8))sc-=300;if(a.token&&b.token&&Math.abs(a.token-b.token)===1)sc-=25;}return sc;}
function shuffleBoard(){const x=s();if(!x||!x.board||!Array.isArray(x.board.hexes))return;const res=["desert","wood","wood","wood","wood","sheep","sheep","sheep","sheep","wheat","wheat","wheat","wheat","brick","brick","brick","ore","ore","ore"],tok=[2,3,3,4,4,5,5,6,6,8,8,9,9,10,10,11,11,12];let best=null,bestSc=-999999;for(let n=0;n<1800;n++){const rs=sh(res),ts=sh(tok);let ti=0;const cand=x.board.hexes.map((old,i)=>{const r=rs[i];return Object.assign({},old,{resource:r,token:r==="desert"?null:ts[ti++]});});const sc=boardScore(cand);if(sc>bestSc){bestSc=sc;best=cand;}}if(best)x.board.hexes=best;if(Array.isArray(x.board.ports)){const ps=sh(["3:1","3:1","3:1","3:1","wood","sheep","wheat","brick","ore"]);x.board.ports.forEach((p,i)=>p.type=ps[i%ps.length]);}x._diceBag=diceBag();x._diceLastTotals=[];robberToDesert(x);}
function hasPieces(x){return !!(x&&x.pieces&&((x.pieces.settlements&&x.pieces.settlements.length)||(x.pieces.roads&&x.pieces.roads.length)||(x.pieces.cities&&x.pieces.cities.length)));}
window.shuffleSettlersBoard=function(){const x=s();if(!x)return;if(!host()){alert("Only the host can shuffle the board.");return;}if(hasPieces(x)){alert("You can only shuffle before anyone places pieces.");return;}x.phase="prestart";if(x.setup){x.setup.active=false;x.setup.stepIndex=0;x.setup.needed="settlement";x.setup.pendingSettlement=null;}shuffleBoard();x.message="Board shuffled. Hit Start Game when ready.";rerender(true);};
window.startSettlersSetupGame=function(){const x=s();if(!x)return;if(!host()){alert("Only the host can start the game.");return;}if(hasPieces(x))return;const n=x.players&&x.players.length?x.players.length:3,o=[];for(let i=0;i<n;i++)o.push(i);for(let i=n-1;i>=0;i--)o.push(i);ensureRobber(x);x.phase="setup";x.turnIndex=o[0]||0;x.rolledThisTurn=false;x.lastRoll=null;x.pendingRoll=null;x.highlightedRoll=null;x.awaitingRobberMove=null;x.setup={active:true,order:o,stepIndex:0,needed:"settlement",pendingSettlement:null};const p=x.players&&x.players[x.turnIndex]?x.players[x.turnIndex]:{name:"Player 1"};x.message=p.name+": choose Settle below for your first settlement.";rerender(true);};
function addPrestartButtons(){const x=s(),area=document.querySelector(".set-map-viewport");if(!x||!area)return;const old=document.getElementById("settlersShuffleStartControls");if(old)old.remove();if(!host()||hasPieces(x))return;if(x.phase!=="prestart"&&x.phase!=="waiting"&&!(x.setup&&x.setup.active===false))return;const d=document.createElement("div");d.id="settlersShuffleStartControls";d.innerHTML='<button type="button" onclick="shuffleSettlersBoard()" style="border:2px solid #fff;border-radius:999px;padding:8px 11px;background:#1d4ed8;color:#fff;font-size:13px;font-weight:900;box-shadow:0 3px 10px rgba(0,0,0,.35);">Shuffle</button><button type="button" onclick="startSettlersSetupGame()" style="border:2px solid #fff;border-radius:999px;padding:8px 11px;background:#ffd700;color:#1e4620;font-size:13px;font-weight:900;box-shadow:0 3px 10px rgba(0,0,0,.35);">Start Game</button>';Object.assign(d.style,{position:"absolute",right:"10px",bottom:"10px",zIndex:"9999",display:"flex",gap:"7px",alignItems:"center"});area.style.position="relative";area.appendChild(d);}
function roadOutlines(){const x=s(),svg=document.querySelector(".set-board-zoomer svg");if(!x||!x.pieces||!Array.isArray(x.pieces.roads)||!svg)return;svg.querySelectorAll(".settlers-road-outline-added").forEach(e=>e.remove());const g=svg.querySelector("#placed-pieces");if(!g)return;x.pieces.roads.forEach(r=>{const l=document.createElementNS("http://www.w3.org/2000/svg","line");l.setAttribute("class","settlers-road-outline-added");l.setAttribute("x1",r.x1);l.setAttribute("y1",r.y1);l.setAttribute("x2",r.x2);l.setAttribute("y2",r.y2);l.setAttribute("stroke","#fff");l.setAttribute("stroke-width","13");l.setAttribute("stroke-linecap","round");l.setAttribute("pointer-events","none");if(g.firstChild)g.insertBefore(l,g.firstChild);else g.appendChild(l);});}
function computer(){const p=cur();return !!(p&&p.isComputer);}
function drawRobber(){const x=s(),svg=document.querySelector(".set-board-zoomer svg"),area=document.querySelector(".set-map-viewport");if(!x||!svg||!x.board||!Array.isArray(x.board.hexes))return;ensureRobber(x);const old=svg.querySelector("#settlersRobberLayer");if(old)old.remove();const op=document.getElementById("settlersRobberPrompt");if(op)op.remove();const ns="http://www.w3.org/2000/svg",layer=document.createElementNS(ns,"g");layer.id="settlersRobberLayer";const wait=x.awaitingRobberMove,idx=x.robber?x.robber.hexIndex:desert(x),can=wait&&(wait.playerId===me()||(computer()&&host()));if(can)x.board.hexes.forEach((h,i)=>{if(i===idx)return;const c=document.createElementNS(ns,"circle");c.setAttribute("cx",h.x);c.setAttribute("cy",h.y);c.setAttribute("r","34");c.setAttribute("fill","rgba(138,43,226,.26)");c.setAttribute("stroke","#fff");c.setAttribute("stroke-width","4");c.setAttribute("style","cursor:pointer");c.setAttribute("onclick","moveSettlersRobber("+i+")");layer.appendChild(c);});const h=x.board.hexes[idx];if(h){const mk=document.createElementNS(ns,"g");mk.setAttribute("pointer-events","none");const sh=document.createElementNS(ns,"circle");sh.setAttribute("cx",h.x+2);sh.setAttribute("cy",h.y+2);sh.setAttribute("r","20");sh.setAttribute("fill","rgba(0,0,0,.28)");mk.appendChild(sh);const cc=document.createElementNS(ns,"circle");cc.setAttribute("cx",h.x);cc.setAttribute("cy",h.y);cc.setAttribute("r","19");cc.setAttribute("fill","#111");cc.setAttribute("stroke","#fff");cc.setAttribute("stroke-width","4");mk.appendChild(cc);const t=document.createElementNS(ns,"text");t.setAttribute("x",h.x);t.setAttribute("y",h.y+7);t.setAttribute("text-anchor","middle");t.setAttribute("font-size","21");t.setAttribute("font-weight","900");t.setAttribute("fill","#fff");t.textContent="R";mk.appendChild(t);layer.appendChild(mk);}svg.appendChild(layer);if(wait&&area){const p=cur(),d=document.createElement("div");d.id="settlersRobberPrompt";d.textContent=wait.playerId===me()?"Move the robber: tap a tile.":(p.name||"Player")+" is moving the robber.";Object.assign(d.style,{position:"absolute",left:"50%",top:"8px",transform:"translateX(-50%)",zIndex:"9999",background:"rgba(0,0,0,.78)",color:"#fff",border:"2px solid #fff",borderRadius:"999px",padding:"7px 12px",fontSize:"13px",fontWeight:"900",textAlign:"center"});area.appendChild(d);}}
function drawExtras(){addPrestartButtons();roadOutlines();drawRobber();}
function pickRobberHex(){const x=s();if(!x||!x.board||!Array.isArray(x.board.hexes))return 0;ensureRobber(x);const now=x.robber.hexIndex,opts=x.board.hexes.map((h,i)=>{const v=h.token===6||h.token===8?8:h.token===5||h.token===9?6:h.token===4||h.token===10?4:h.token===3||h.token===11?2:h.token===2||h.token===12?1:0;return{index:i,score:v+(h.resource==="desert"?-6:0)};}).filter(o=>o.index!==now).sort((a,b)=>b.score-a.score).slice(0,5);return opts.length?opts[rnd(opts.length)].index:0;}
window.moveSettlersRobber=function(i){const x=s();if(!x||!x.awaitingRobberMove||!x.board||!Array.isArray(x.board.hexes))return;const p=cur(),ok=x.awaitingRobberMove.playerId===me()||(p&&p.isComputer&&host());if(!ok)return;i=Number(i);if(!x.board.hexes[i])return;if(x.robber&&i===x.robber.hexIndex)return;x.robber={hexIndex:i};x.awaitingRobberMove=null;x.message=(p.name||"Player")+" moved the robber.";rerender(true);};
function autoRobber(){const x=s(),p=cur();if(x&&x.awaitingRobberMove&&p&&p.isComputer&&host())window.moveSettlersRobber(pickRobberHex());}
function finish7(id){const x=s();if(!x||!x.pendingRoll||x.pendingRoll.id!==id)return;const p=cur();x.pendingRoll.applied=true;x.pendingRoll=null;x.highlightedRoll=null;x.awaitingRobberMove={playerId:p.id,turnNumber:x.turnNumber||1,rollKey:id};x._robberHandledRollKey=id;x.message=(p.name||"Player")+" rolled 7. Move the robber.";rerender(true);if(p.isComputer&&host())setTimeout(autoRobber,900);}
function finishRoll(id){const x=s();if(!x||!x.pendingRoll||x.pendingRoll.id!==id)return;const g=x.pendingRoll.gains||{};applyG(g);x.resourceBurst={id:"burst-"+Date.now()+"-"+rnd(9999),gains:JSON.parse(JSON.stringify(g))};x.pendingRoll.applied=true;x.pendingRoll=null;x.highlightedRoll=null;x.message=cur().name+" rolled "+(x.lastRoll?x.lastRoll.total:"")+".";rerender(true);}
window.rollSettlersDice=function(){const x=s();if(!x)return;ensureRobber(x);if(x.setup&&x.setup.active){if(window.setSettlersMessage)window.setSettlersMessage("Finish setup before rolling dice.");return;}const p=cur();if(!p||p.id!==me()){if(window.setSettlersMessage)window.setSettlersMessage("Waiting for "+(p?p.name:"Player")+"'s turn.");return;}if(x.awaitingRobberMove){if(window.setSettlersMessage)window.setSettlersMessage("Move the robber first.");drawRobber();return;}if(x.pendingRoll){if(window.setSettlersMessage)window.setSettlersMessage("Wait for the dice roll to finish.");return;}if(x.rolledThisTurn){if(window.setSettlersMessage)window.setSettlersMessage("You already rolled. Build, trade, buy, or end your turn.");return;}const r=rollFromBag(x),id="roll-"+Date.now()+"-"+rnd(9999);x.lastRoll={d1:r.d1,d2:r.d2,total:r.total};x.rolledThisTurn=true;x.highlightedRoll=r.total;x.pendingRoll={id:id,d1:r.d1,d2:r.d2,total:r.total,gains:r.total===7?{}:gains(r.total),applied:false,robberPatched:true};x.message=p.name+" rolled "+r.d1+" + "+r.d2+" = "+r.total+".";rerender(true);if(r.total===7)setTimeout(()=>finish7(id),1400);else setTimeout(()=>finishRoll(id),3000);};
function blockMsg(){if(window.setSettlersMessage)window.setSettlersMessage("Move the robber first.");drawRobber();}
function wrap(name){const old=window[name];if(typeof old!=="function"||old.__robberBlockWrapped)return;window[name]=function(){const x=s();if(x&&x.awaitingRobberMove){blockMsg();return;}return old.apply(this,arguments);};window[name].__robberBlockWrapped=true;}
["placeSettlement","placeRoad","placeCity","buySettlersDevelopmentCard","endSettlersTurn","openSettlersTrade","openSettlersPlayerTrade","setSettlersUiState"].forEach(wrap);
function watchOldRolls(){const x=s();if(!x)return;ensureRobber(x);if(x.pendingRoll&&!x.pendingRoll.robberPatched){x.pendingRoll.gains=x.pendingRoll.total===7?{}:gains(x.pendingRoll.total);x.pendingRoll.robberPatched=true;}if(x.phase==="playing"&&x.rolledThisTurn&&x.lastRoll&&x.lastRoll.total===7&&!x.pendingRoll&&!x.awaitingRobberMove){const p=cur(),key="old-"+(x.turnNumber||1)+"-"+p.id+"-"+x.lastRoll.d1+"-"+x.lastRoll.d2;if(x._robberHandledRollKey!==key){x._robberHandledRollKey=key;x.awaitingRobberMove={playerId:p.id,turnNumber:x.turnNumber||1,rollKey:key};x.message=(p.name||"Player")+" rolled 7. Move the robber.";rerender(true);if(p.isComputer&&host())setTimeout(autoRobber,900);}}}
const oldInit=window.initSettlersGame;if(typeof oldInit==="function"&&!oldInit.__shuffleStartDiceRobberWrapped){window.initSettlersGame=function(){const out=oldInit.apply(this,arguments),x=s();if(x){ensureRobber(x);if(!x.__shuffleStartDiceRobberInitialized&&!hasPieces(x)){x.__shuffleStartDiceRobberInitialized=true;x.phase="prestart";if(x.setup)x.setup.active=false;x.rolledThisTurn=false;x.lastRoll=null;x.pendingRoll=null;x.highlightedRoll=null;x.awaitingRobberMove=null;x._diceBag=diceBag();x._diceLastTotals=[];robberToDesert(x);x.message=host()?"Shuffle the board until you like it, then start the game.":"Waiting for the host to start the game.";}rerender(true);}setTimeout(drawExtras,80);return out;};window.initSettlersGame.__shuffleStartDiceRobberWrapped=true;window.startSettlersFromLobby=window.initSettlersGame;window.startSettlersGame=window.initSettlersGame;}
const canvas=document.getElementById("gameCanvasContainer");if(canvas)new MutationObserver(drawExtras).observe(canvas,{childList:true,subtree:true});setInterval(()=>{watchOldRolls();drawExtras();},700);
})();
