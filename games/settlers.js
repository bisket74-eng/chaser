...
function setMessage(text) {
    ...
    if (current) {
        current.textContent = topEntry ? topEntry.text : getSettlersHeaderText();
    } else {
...
function logSettlersEvent(text) {
    const st = window.settlersState;
    if (!st || !text) return;
...
function applyPendingSettlersRoll(rollId) {
    ...
    if (total === 7) {
        ...
        logSettlersEvent(`${roller.name} rolled ${st.pendingRoll.d1} + ${st.pendingRoll.d2} = 7. Robber must move.`);
        st.pendingRoll = null;
    ...
    logSettlersEvent(`${roller.name} rolled ${st.pendingRoll.d1} + ${st.pendingRoll.d2} = ${total}.${gainsBits.length ? " " + gainsBits.join("; ") + "." : " No one collected resources."}`);
...
function endCurrentTurn() {
    ...
    logSettlersEvent(`${current.name}'s turn began.`);
