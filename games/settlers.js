...
// device can expand/collapse its own log independently.
...
function setMessage(text) {
    ...
    const current = msg.querySelector(".set-log-current");
    const log = st && Array.isArray(st.gameLog) ? st.gameLog : [];
    const topEntry = log.length ? log[log.length - 1] : null;

    // If there's a logged action already showing, a transient validation
    // message (like "roll first") would otherwise clobber it. Only override
    // the header text directly when there is no log entry to show instead.
    // Rebuilt with innerHTML (not textContent) so a turn-start dot survives.
    if (current) {
        const dot = topEntry ? logDotHtml(topEntry.dotPlayerId) : "";
        const displayText = topEntry ? topEntry.text : getSettlersHeaderText();
        current.innerHTML = dot + escapeHtml(displayText);
    } else {
        msg.innerText = getSettlersHeaderText();
    }
}
...
// Pops the most recent log entry into view by simply ensuring a re-render
// has happened; the header always displays the latest gameLog entry, so
// "popping" is just: log it, then render. Kept as a named hook in case a
// future pass wants a distinct flash/highlight animation on new entries.
...
function applyPendingSettlersRoll(rollId) {
    ...
    const roller = currentSettlersPlayer();
    st.message = `Rolled a ${total}. ${summarizeMyGains(gains)}`;
    ...
    logSettlersEvent(gainsBits.length ? gainsBits.join("; ") + "." : "No one collected resources.");
...
function buildSettlersLogHeaderHtml(topLogEntry) {
    const st = window.settlersState;
    const headerText = topLogEntry ? topLogEntry.text : getSettlersHeaderText();
    const headerDot = topLogEntry ? logDotHtml(topLogEntry.dotPlayerId) : "";
    const log = (st && Array.isArray(st.gameLog)) ? st.gameLog : [];
...
function renderSettlers() {
    ...
    el.innerHTML = `
        <style>
            ...
            .set-log-dot { display:inline-block; width:10px; height:10px; border-radius:999px; margin-right:7px; border:1.5px solid rgba(255,255,255,0.85); vertical-align:middle; flex-shrink:0; }
        ...
        <div class="set-wrap">
            ${buildSettlersLogHeaderHtml(topLogEntry)}
