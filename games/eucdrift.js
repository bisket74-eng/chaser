    ...
    container.innerHTML = `
        ...
        <div id="eucDriftRoot">
            ...
            <div id="eucDriftHud">
                ...
                <div id="eucLivesHud" style="position:absolute;left:50%;transform:translateX(-50%);top:52px;font-size:13px;letter-spacing:2px;pointer-events:none;">❤️❤️❤️</div>
                <div id="eucSpeedWrap">
            ...
            <div id="eucOverheatBanner" class="eucHidden">⚠ MOTOR CUTOUT WARNING — EASE OFF</div>
            <div id="eucHillBanner" class="eucHidden"></div>

            <!-- Battery bar -->
            <div id="eucBatteryWrap" style="position:absolute;bottom:88px;right:14px;z-index:11;pointer-events:none;display:flex;flex-direction:column;align-items:flex-end;gap:3px;">
                <div style="font-size:9px;font-weight:700;color:rgba(234,242,248,0.5);letter-spacing:0.8px;">BATTERY</div>
                <div id="eucBatteryTrack" style="width:52px;height:10px;background:rgba(255,255,255,0.1);border-radius:5px;overflow:hidden;border:1px solid rgba(255,255,255,0.15);">
                    <div id="eucBatteryFill" style="height:100%;width:100%;background:#4fd1c5;border-radius:5px;transition:background 0.3s;"></div>
                </div>
            </div>

            <!-- Coin counter -->
            <div id="eucCoinHud" style="position:absolute;top:10px;left:50%;transform:translateX(-50%);z-index:11;pointer-events:none;display:flex;flex-direction:column;align-items:center;gap:1px;">
                <div style="font-size:9px;font-weight:700;color:rgba(234,242,248,0.5);letter-spacing:0.8px;">COINS</div>
                <div id="eucCoinCount" style="font-size:18px;font-weight:800;color:#ffd700;text-shadow:0 1px 4px rgba(0,0,0,0.5);">0</div>
            </div>
            ...
            <div class="eucOverlay eucHidden" id="eucGameOverOverlay">
                ...
                <div id="eucLivesDisplay" style="font-size:22px;margin-bottom:8px;letter-spacing:4px;">❤️❤️❤️</div>
...
window.__eucDriftRunGame = function () {
    ...
    const game = {
        ...
        // Coins
        coins: 0,
        totalCoins: parseInt(safeLocalGet("eucdrift_coins", "0")) || 0,
        // Near-miss bonus
        nearMissFlash: null,
        // Battery bar (replaces sudden-death overheat)
        batteryCharge: 1.0,
        // Finish lines
        zonesCompleted: 0,
        finishLineWorldX: -1,
    ...
    // Curve system constants
    const CURVE_MAX_OFFSET   = 60;   // max px the road can shift before you're "off road"
    const CURVE_DRIFT_RATE   = 48;   // px/s the road drifts when player isn't steering
    const CURVE_RECOVER_RATE = 90;   // px/s the road snaps back when player holds correct dodge
    const CURVE_SPEED_PENALTY = 0.7; // speedNorm loss per second when drifting over 50% off-line
    const CURVE_MIN_STRAIGHT  = 4.0; // seconds of straight road between curves
    const CURVE_DURATION_MIN  = 2.5; // minimum curve duration in seconds
    const CURVE_DURATION_MAX  = 5.0; // maximum curve duration in seconds
    ...
    function setupPad() {
        ...
        // Also tracks hold state for curve steering — a curve requires
        // holding the correct direction continuously, not just tapping.
        game._dodgeUpHeld = false;
        game._dodgeDownHeld = false;
        ...
        dodgeMap.forEach(([id, dir]) => {
            ...
            const down = (e) => {
                ...
                triggerDodge(dir);
                if (dir > 0) game._dodgeUpHeld = true;
                else game._dodgeDownHeld = true;
            };
            const up = (e) => {
                if (e) e.preventDefault();
                el.classList.remove("active");
                if (dir > 0) game._dodgeUpHeld = false;
                else game._dodgeDownHeld = false;
            };
        ...
        // Haptic: single buzz on contact touch only — no continuous vibration
        function startHaptic(intensity) { /* slider haptics removed */ }
        function stopHaptic() { /* slider haptics removed */ }
        ...
        function onSliderStart(clientX) {
            ...
            onSliderMove(clientX);
        }

        function onSliderMove(clientX) {
            if (!sliderActive) return;
            // Absolute position mode: track center = 0, full right = +1 (max
            // forward), full left = -1 (max reverse). Wherever your finger
            // rests on the slider IS your speed — no drift up or down as
            // long as you hold still. Lift your finger and it springs to
            // center (0) and speed decays naturally.
    ...
    const MAX_REVERSE_SPEED_NORM = -0.60; // ~30 mph reverse
    ...
    function updateOverheatState(dt) {
        // Battery charge bar (0..1) — visible to player as the bar in the HUD.
        // High speed drains it; easing off recharges it. Hold max speed for
        // ~10 seconds and it hits zero → motor cutout → you go flying.
        // Warning banner appears below 25% so there's time to react.
        const speedFraction = Math.max(0, game.speedNorm / 1.3); // 0..1
        const onSteepDescent = hillSlopeAt(game.distance) < -0.3 && game.speedNorm > 0.5;

        if (speedFraction > 0.6 || onSteepDescent) {
            // Drain rate scales with BOTH speed AND how low the battery already is.
            // Full battery + max speed ≈ 10 seconds before cutout.
            // Half battery + max speed ≈ 4-5 seconds.
            // Quarter battery + max speed ≈ 2 seconds — barely any tolerance left.
            const chargeMultiplier = 1 + (1 - game.batteryCharge) * 2.5; // 1x at full, 3.5x at empty
            const drainRate = (0.04 + speedFraction * 0.06) * chargeMultiplier;
            game.batteryCharge = Math.max(0, game.batteryCharge - drainRate * dt);
        } else if (speedFraction < 0.3) {
            // Recharges when coasting or slowing — gives player a real strategy
            const rechargeRate = 0.09;
            game.batteryCharge = Math.min(1, game.batteryCharge + rechargeRate * dt);
        }

        // Warning banner when below 25% — gives time to ease off
        rider.overheatWarning = game.batteryCharge < 0.25 && game.batteryCharge > 0;

        // Flash the front warning light while in the danger zone
        if (rider.overheatWarning) {
            rider.beepFlash = 0.3;
        } else if (rider.beepFlash > 0) {
            rider.beepFlash -= dt;
        }

        // Empty battery → motor cutout → crash. This is the real consequence.
        // Only triggers if actually moving at speed (not if you already slowed down in time).
        if (game.batteryCharge <= 0 && !rider.crashed && !rider.cutoutFlying) {
            game.batteryCharge = 0;
            if (game.speedNorm > 0.3) {
                // Going fast enough that the cutout throws you — Superman crash
                triggerOverheatCutout();
            } else {
                // Already slowed down enough — just stall, no flying crash
                game.speedNorm = 0;
                game.batteryCharge = 0.05; // tiny bump so it doesn't re-trigger immediately
            }
        }
    }

    function triggerOverheatCutout() {
        // Motor cuts out at speed — rider flies forward (Superman pose) into a crash.
        // This is what happens on a real EUC when the battery protection kicks in.
        if (rider.crashed) return;
    ...
    const INJURY_MESSAGES = [
        null,                    // placeholder (lives go 3→2→1→0)
        "You broke your leg — you're done!",   // 0 lives left = game over
        "You broke your wrist! Get back on!",  // 1 life left
        "You broke your arm! Shake it off!",   // 2 lives left
    ];
    ...
    function crashRider(reason) {
        ...
        rider.hitFlash = 0.6;
        // Haptic crash buzz — two-pulse impact feel
        if (navigator.vibrate) navigator.vibrate([80, 40, 120]);

        game.lives--;

        if (game.lives <= 0) {
            // All lives gone — proper game over
            endGame(true);
        } else {
            // Still have lives — show injury message then respawn
            endGame(false);
        }
    }

    function respawnRider() {
        game.distance = game.checkpointDistance;
        world.scrollX = game.checkpointDistance / (game.baseScroll * 0.06);
        game.speedNorm = 0;
        game.zoneIndex = game.checkpointZone;
        game.trickScoreFlash = null;
        game.nearMissFlash = null;
        game.batteryCharge = 1.0;
        // Reset curve state so you don't respawn mid-curve
        game.curveOffset = 0;
        game.curveDir = 0;
        game.curveDrift = 0;
        game.curveTimer = 0;
        game.straightTimer = 3.0; // grace period before first curve
        game._curveTooFastWarning = 0;
        game._dodgeUpHeld = false;
        game._dodgeDownHeld = false;

        obstacles.length = 0;
        coins.length = 0;
        nextObstacleAt = world.scrollX + W + 200;
        nextCoinGroupAt = world.scrollX + W + 300;
    ...
    function riderGroundY() {
        return groundY() + laneCenterOffset(rider.laneT) + game.curveOffset;
    }
    ...
    function drawRider() {
        ...
        const curveTilt = (!rider.airborne && !rider.crashed)
            ? (game.curveDrift / CURVE_MAX_OFFSET) * -0.18  // lean into the curve
            : 0;
        const leanAngle = rider.airborne
            ? Math.min(0.5, (rider.tricks.length * 0.35)) * Math.sin(performance.now() / 120)
            : rider.lean * 0.22 + hillPitch + curveTilt;
    ...
    function updateZone() {
        const newZoneIndex = Math.floor(game.distance / ZONE_LENGTH) % game.zoneNames.length;
        // Save checkpoint whenever we reach a zone we haven't been to yet
        // this run — so dying in countryside respawns at countryside start,
        // not back at city.
        if (newZoneIndex > game.checkpointZone) {
            game.checkpointZone = newZoneIndex;
            game.checkpointDistance = newZoneIndex * ZONE_LENGTH;
        }
        game.zoneIndex = newZoneIndex;
    }
    ...
    function drawBackground() {
        ...
        // curveOffset shifts the road band up/down to simulate curving.
        // Positive = road shifts down (right curve), negative = shifts up (left curve).
        const bandTop = laneBandTop() + game.curveOffset;
        const bandH = laneBandHeight();
        const bandMid = bandTop + bandH / 2;

        // ground below and above the shifted road band
        ctx.fillStyle = zone === "track" ? "#191220" : (zone === "countryside" ? "#2e3a28" : "#15191d");
        ctx.fillRect(0, bandTop + bandH, W, H - (bandTop + bandH));
        ctx.fillRect(0, 0, W, bandTop); // sky/ground above the road

        // road surface band (covers both lanes)
        ctx.fillStyle = zone === "track" ? "#2a2138" : "#21262b";
        ctx.fillRect(0, bandTop, W, bandH);

        // outer edge lines — flash red when drifting off
        ...
        // Curve indicator — shows which way to lean AND warns if going too fast
        const curveHint = getCurveIndicatorText();
        if (curveHint || (game._curveTooFastWarning > 0)) {
            const driftFraction = Math.abs(game.curveDrift) / CURVE_MAX_OFFSET;
            ctx.globalAlpha = Math.min(1, driftFraction * 2 + 0.3);
            if (game._curveTooFastWarning > 0.4) {
                // Speed is the primary problem — warn about that
                ctx.fillStyle = "#ff4444";
                ctx.font = "bold 15px -apple-system, sans-serif";
                ctx.textAlign = "center";
                ctx.fillText("⚠ TOO FAST — SLOW DOWN!", W / 2, bandTop - 8);
            } else if (curveHint) {
                ctx.fillStyle = driftFraction > 0.6 ? "#ff4444" : "#ffd700";
                ctx.font = "bold 14px -apple-system, sans-serif";
                ctx.textAlign = "center";
                ctx.fillText(curveHint, W / 2, bandTop - 8);
            }
            ctx.textAlign = "left";
            ctx.globalAlpha = 1;
        }
    ...
    const obstacles = [];
    const coins = [];           // { x, lane, collected, bobT }
    let nextCoinGroupAt = 600;  // world-x of next coin group spawn
    ...
    function spawnCoinGroup() {
        // Spawn a line of 5-8 coins in one lane, or alternating both lanes
        const lane = Math.random() < 0.5 ? LANE_TOP : LANE_BOTTOM;
        const count = 5 + Math.floor(Math.random() * 4);
        const spacing = 55;
        const pattern = Math.random(); // 0=single lane, 0.3=zigzag, 0.6=both lanes
        const startX = world.scrollX + W + 100;

        for (let i = 0; i < count; i++) {
            let coinLane = lane;
            if (pattern > 0.6) {
                coinLane = i % 2 === 0 ? LANE_TOP : LANE_BOTTOM;
            } else if (pattern > 0.3) {
                coinLane = Math.random() < 0.5 ? LANE_TOP : LANE_BOTTOM;
            }
            coins.push({
                x: startX + i * spacing,
                lane: coinLane,
    ...
    function updateCoins(dt) {
        // Spawn coin groups ahead
        if (world.scrollX + W > nextCoinGroupAt) {
            spawnCoinGroup();
            // Gap between groups: 350-600px, tightens slightly with distance
            nextCoinGroupAt = world.scrollX + W + 350 + Math.random() * 250;
        }

        // Prune old coins far behind
        const pruneBehind = world.scrollX - W * 2;
        for (let i = coins.length - 1; i >= 0; i--) {
            if (coins[i].x < pruneBehind) coins.splice(i, 1);
        }

        // Animate bob
        coins.forEach(c => { c.bobT += dt * 3.2; });

        // Check collection
        if (!rider.crashed && !rider.airborne) {
            const rx = rider.x;
            const rl = rider.lane;
            coins.forEach(c => {
                ...
                if (Math.abs(sx - rx) < COIN_COLLECT_RADIUS) {
                    c.collected = true;
                    game.coins++;
                    game.totalCoins++;
                    safeLocalSet("eucdrift_coins", String(game.totalCoins));
                    // small distance bonus per coin
                    game.distance += 2;
                }
            });
        }
    ...
    function drawCoins() {
        const zoneColors = {
            city: { fill: "#ffd700", rim: "#ffaa00", shine: "rgba(255,255,220,0.8)" },
            countryside: { fill: "#7ec850", rim: "#5a9e38", shine: "rgba(200,255,180,0.7)" },
            track: { fill: "#b06aff", rim: "#8040cc", shine: "rgba(220,180,255,0.8)" },
        };
        const zc = zoneColors[currentZoneName()] || zoneColors.city;
        ...
        coins.forEach(c => {
            ...
            const gy = groundY() + laneCenterOffset(c.lane === LANE_TOP ? 0 : 1) + game.curveOffset;
            const bob = Math.sin(c.bobT) * 4;
            const cy = gy - COIN_RADIUS * 2.4 + bob;

            // Coin body
            ctx.beginPath();
            ctx.arc(sx, cy, COIN_RADIUS, 0, Math.PI * 2);
            ctx.fillStyle = zc.fill;
            ctx.fill();
            ctx.strokeStyle = zc.rim;
            ctx.lineWidth = 2;
            ctx.stroke();

            // Shine highlight
            ctx.beginPath();
            ctx.arc(sx - 2.5, cy - 2.5, COIN_RADIUS * 0.38, 0, Math.PI * 2);
            ctx.fillStyle = zc.shine;
            ctx.fill();

            // $ symbol
            ctx.fillStyle = zc.rim;
            ctx.font = `bold ${Math.round(COIN_RADIUS * 1.1)}px -apple-system, sans-serif`;
            ctx.textAlign = "center";
    ...
    // FINISH LINES
    // ============================================================
    const ZONE_FINISH_OFFSET = 680; // how far into a zone the finish line appears

    function updateFinishLine(dt) {
        if (game.finishFlash > 0) game.finishFlash -= dt;

        // Place a finish line near the end of each zone
        const posInZone = game.distance % ZONE_LENGTH;
        const zoneStart = Math.floor(game.distance / ZONE_LENGTH) * ZONE_LENGTH;
        const finishDist = zoneStart + ZONE_FINISH_OFFSET;

        // Set up the finish line world-x when we're approaching it
        if (posInZone > ZONE_FINISH_OFFSET - 200 && !game.finishLineCrossed) {
            if (game.finishLineWorldX < zoneStart) {
                // Convert finish line distance to world scroll position
                game.finishLineWorldX = finishDist / (game.baseScroll * 0.06);
                game.finishLineCrossed = false;
            }
        }

        // Reset for new zone
        if (posInZone < 50 && game.finishLineCrossed) {
            game.finishLineCrossed = false;
            game.finishLineWorldX = -1;
        }

        // Check if rider crossed the finish line
        if (!game.finishLineCrossed && game.finishLineWorldX > 0) {
            const finishSX = game.finishLineWorldX - world.scrollX;
            if (finishSX < rider.x && !rider.crashed) {
                game.finishLineCrossed = true;
                game.zonesCompleted++;
                game.finishFlash = 2.0;
                // Big coin bonus for completing a zone
                const bonus = 50 * (game.zonesCompleted);
                game.coins += bonus;
                game.totalCoins += bonus;
                safeLocalSet("eucdrift_coins", String(game.totalCoins));
                game.distance += bonus * 0.5;
                // Save checkpoint
                if (game.zoneIndex > game.checkpointZone) {
                    game.checkpointZone = game.zoneIndex;
                    game.checkpointDistance = game.zoneIndex * ZONE_LENGTH;
                }
            }
        }
    }
    ...
    function drawFinishLine() {
        if (game.finishLineWorldX < 0) return;
        const sx = game.finishLineWorldX - world.scrollX;
        ...
        // Checkered pattern
        const sqSize = 14;
        const cols = Math.ceil(bandH / sqSize);
        for (let i = 0; i < cols; i++) {
            ctx.fillStyle = i % 2 === 0 ? "#ffffff" : "#000000";
            ctx.fillRect(sx - sqSize, bandTop + i * sqSize, sqSize, sqSize);
            ctx.fillStyle = i % 2 === 0 ? "#000000" : "#ffffff";
            ctx.fillRect(sx, bandTop + i * sqSize, sqSize, sqSize);
        }

        // Finish line poles
        ctx.fillStyle = "#cccccc";
        ctx.fillRect(sx - sqSize - 4, bandTop - 20, 4, bandH + 20);
        ctx.fillRect(sx + sqSize, bandTop - 20, 4, bandH + 20);

        // Ribbon banners at top
        ctx.fillStyle = "#ff3366";
        ctx.fillRect(sx - sqSize - 4, bandTop - 20, sqSize * 2 + 8, 10);
    }

    function drawFinishFlash() {
        if (game.finishFlash <= 0) return;
        const alpha = Math.min(1, game.finishFlash) * 0.35;
        ctx.fillStyle = `rgba(255,215,0,${alpha})`;
        ctx.fillRect(0, 0, W, H);

        if (game.finishFlash > 1.2) {
            ctx.fillStyle = `rgba(255,255,255,${(game.finishFlash - 1.2) * 0.8})`;
            ctx.font = "bold 28px -apple-system, sans-serif";
            ctx.textAlign = "center";
            const zoneName = game.zoneNames[(game.zoneIndex - 1 + game.zoneNames.length) % game.zoneNames.length];
            const label = zoneName.charAt(0).toUpperCase() + zoneName.slice(1);
            ctx.fillText(`✓ ${label} Complete!`, W / 2, H / 2 - 20);
            const bonus = 50 * game.zonesCompleted;
            ctx.font = "bold 18px -apple-system, sans-serif";
            ctx.fillStyle = `rgba(255,215,0,${(game.finishFlash - 1.2) * 0.8})`;
            ctx.fillText(`+${bonus} coins!`, W / 2, H / 2 + 14);
        }
        ctx.textAlign = "left";
    }
    ...
    function checkNearMisses() {
        ...
        const NEAR_DIST = 55; // px — within this of an obstacle edge counts as a near miss

        for (const o of obstacles) {
            if (o.type === "ramp") continue;
            if (o.lane === rider.lane) continue; // would be a hit, not a miss
            const sx = obstacleScreenX(o);
            const dist = Math.abs(sx + o.w / 2 - rider.x);
            if (dist < NEAR_DIST && !o.nearMissed) {
                o.nearMissed = true;
                game.nearMissFlash = { timer: 1.2 };
                game.distance += 5;
                game.coins += 3;
                game.totalCoins += 3;
                safeLocalSet("eucdrift_coins", String(game.totalCoins));
    ...
    function drawNearMissFlash() {
        if (!game.nearMissFlash || game.nearMissFlash.timer <= 0) return;
        game.nearMissFlash.timer -= (1 / 60);
        const a = Math.min(1, game.nearMissFlash.timer / 0.6);
        ctx.globalAlpha = a;
        ctx.fillStyle = "#00ffcc";
        ctx.font = `bold ${Math.round(15 * riderScale())}px -apple-system, sans-serif`;
        ctx.textAlign = "center";
        ctx.fillText("NEAR MISS! +3", rider.x, riderGroundY() - RIDER_HEIGHT * riderScale() - 38);
        ctx.globalAlpha = 1;
        ctx.textAlign = "left";
    }
    ...
    // CURVE SYSTEM
    // ============================================================
    // Curves are stretches of road that pull the player sideways.
    // To stay on road, hold the correct dodge direction the whole
    // time. Let go and you drift; drift too far and you crash.
    // The road visually shifts up/down, background shifts opposite,
    // and the rider tilts into the curve for a lean-in feel.

    function updateCurves(dt) {
        if (rider.crashed || rider.airborne) return;

        // Spawn logic — new curve or countdown to next.
        // Track bonus round gets tighter, more frequent curves.
        const isTrack = currentZoneName() === "track";
        const minStraight = isTrack ? 2.0 : CURVE_MIN_STRAIGHT;
        const durationMin = isTrack ? 3.0 : CURVE_DURATION_MIN;
        const durationMax = isTrack ? 7.0 : CURVE_DURATION_MAX;

        if (game.curveDir === 0) {
            game.straightTimer -= dt;
            if (game.straightTimer <= 0) {
                game.curveDir = Math.random() < 0.5 ? 1 : -1;
                game.curveTimer = durationMin + Math.random() * (durationMax - durationMin);
                game.curveDrift = 0;
            }
        } else {
            // Active curve
            ...
            // The correct counter-input: curve pulling down (+1) → player
            // must hold UP (dodge toward top lane); curve up (-1) → hold DOWN
            const correctInput = game.curveDir > 0
                ? (input.leanLeft === false && document.getElementById("eucPadUp")?.classList.contains("active"))
                : (input.leanLeft === false && document.getElementById("eucPadDown")?.classList.contains("active"));

            // Simpler: check rider's targetLaneT movement direction as a
            // proxy for "is the player holding the right dodge"
            // Actually track dodge button state via a dedicated flag
            const holdingCorrect = game.curveDir > 0
                ? game._dodgeUpHeld
                : game._dodgeDownHeld;

            // Centrifugal force: the faster you're going, the harder the
            // curve pulls you outward. At low speed a curve is manageable
            // even without perfect steering. At high speed it throws you
            // off regardless — you MUST slow down for tight curves.
            // Safe speed for a curve is roughly 40% of max (~26 mph).
            // Above that, centrifugal force adds to the drift rate.
            const speedFraction = Math.max(0, game.speedNorm / 1.3); // 0..1
            const safeCurveSpeed = 0.35; // ~26 mph
            const speedExcess = Math.max(0, speedFraction - safeCurveSpeed);
            const centrifugalExtra = speedExcess * 2.2; // extra drift per second per unit excess speed

            // Total drift rate = base + centrifugal. Holding correct
            // dodge reduces drift but can't fully cancel centrifugal
            // force at extreme speeds — you MUST slow down.
            const totalDriftRate = CURVE_DRIFT_RATE + centrifugalExtra * CURVE_MAX_OFFSET;

            if (holdingCorrect) {
                // Player countering — reduce drift, but centrifugal force
                // still pushes out if going too fast
                const netRate = centrifugalExtra * CURVE_MAX_OFFSET * 0.6 - CURVE_RECOVER_RATE;
                if (netRate > 0) {
                    // Going so fast that even perfect steering can't hold —
                    // drift still grows, just more slowly
                    game.curveDrift += game.curveDir * netRate * dt;
                } else {
                    // Steering wins — drift back toward center
                    game.curveDrift += game.curveDir * netRate * dt;
                    if (Math.sign(game.curveDrift) !== Math.sign(game.curveDir)) {
                        game.curveDrift = 0; // don't overshoot center
                    }
                }
            } else {
                // Not steering — full drift + centrifugal
                game.curveDrift += game.curveDir * totalDriftRate * dt;
            }

            // Speed warning: flash a message when going dangerously fast
            // into a curve before it becomes a crash
            const driftFraction = Math.abs(game.curveDrift) / CURVE_MAX_OFFSET;
            if (speedExcess > 0.15 && game.curveDir !== 0) {
                game._curveTooFastWarning = (game._curveTooFastWarning || 0) + dt;
            } else {
                game._curveTooFastWarning = 0;
            }

            // Progressive speed penalty when drifting — the road is
            // fighting you, scrubbing speed
            if (driftFraction > 0.4 && !rider.crashed) {
                game.speedNorm = Math.max(0, game.speedNorm - CURVE_SPEED_PENALTY * dt);
            }

            // Crash when fully drifted off road
            if (Math.abs(game.curveDrift) >= CURVE_MAX_OFFSET && !rider.crashed) {
                game.curveDrift = Math.sign(game.curveDrift) * CURVE_MAX_OFFSET;
                crashRider("offRoad");
            }

            // Smoothly animate the road offset toward the current drift
            const targetOffset = game.curveDrift;
            game.curveOffset += (targetOffset - game.curveOffset) * Math.min(1, dt * 5);

            // Curve ends — straighten out gradually
            if (game.curveTimer <= 0) {
                game.curveDir = 0;
                game.curveDrift = 0;
                game._curveTooFastWarning = 0;
                game.straightTimer = CURVE_MIN_STRAIGHT + Math.random() * 3;
            }
        }

        // Always ease curveOffset back toward 0 when straight
        if (game.curveDir === 0) {
            game.curveOffset += (0 - game.curveOffset) * Math.min(1, dt * 4);
        }
    }

    function drawCurveEdges() {
        // Rumble strips on both road edges — get redder the closer
        // you are to drifting off
        if (game.curveDir === 0 && Math.abs(game.curveOffset) < 4) return;

        const bandTop = laneBandTop() + game.curveOffset;
        const bandH = laneBandHeight();
        const driftFraction = Math.abs(game.curveDrift) / CURVE_MAX_OFFSET;
        if (driftFraction < 0.3) return;

        const alpha = (driftFraction - 0.3) / 0.7;
        ctx.fillStyle = `rgba(255,50,50,${alpha * 0.45})`;
        // Flash the edge in the drift direction
        if (game.curveDrift > 0) {
    ...
    // Expose curve indicator to the HUD — which direction to lean
    function getCurveIndicatorText() {
        if (game.curveDir === 0) return null;
        if (Math.abs(game.curveDrift) < CURVE_MAX_OFFSET * 0.1) return null;
        return game.curveDir > 0 ? "↑ LEAN UP" : "↓ LEAN DOWN";
    }
    ...
    function obstacleLaneY(o) {
        ...
        return groundY() + laneCenterOffset(laneT) + game.curveOffset;
    }
    ...
    const el = {
        ...
        livesDisplay: document.getElementById("eucLivesDisplay"),
        livesHud: document.getElementById("eucLivesHud"),
        batteryFill: document.getElementById("eucBatteryFill"),
        coinCount: document.getElementById("eucCoinCount"),
    };
    ...
    function updateHUD() {
        ...
        if (el.livesHud) {
            el.livesHud.textContent = "❤️".repeat(game.lives) + "🖤".repeat(3 - game.lives);
        }

        // Battery bar
        if (el.batteryFill) {
            const pct = Math.round(game.batteryCharge * 100);
            el.batteryFill.style.width = pct + "%";
            el.batteryFill.style.background =
                game.batteryCharge > 0.5 ? "#4fd1c5" :
                game.batteryCharge > 0.25 ? "#ffd700" : "#ff4444";
        }

        // Coin counter
        if (el.coinCount) {
            el.coinCount.textContent = game.coins;
        }
    }
    ...
    function startGame() {
        ...
        game.trickScoreFlash = null;
        game.lives = 3;
        game.checkpointZone = 0;
        game.checkpointDistance = 0;
        game.coins = 0;
        game.nearMissFlash = null;
        game.batteryCharge = 1.0;
        game.zonesCompleted = 0;
        game.finishLineWorldX = -1;
        game.finishLineCrossed = false;
        game.finishFlash = 0;
        // Curve state
        game.curveOffset = 0;      // current road vertical shift in px (positive = shifted down)
        game.curveDir = 0;         // -1 = curving up (hold DOWN), +1 = curving down (hold UP), 0 = straight
        game.curveDrift = 0;       // accumulated drift — how far off-line player has let it get
        game.curveTimer = 0;       // time remaining in current curve segment
        game.straightTimer = 0;    // cooldown between curves
        world.scrollX = 0;
        obstacles.length = 0;
        coins.length = 0;
        nextObstacleAt = 900;
    ...
    function endGame(isGameOver) {
        ...
        // Injury message with what caused it
        if (el.crashReason) {
            const causeText = {
                hit:     "You hit an obstacle.",
                offRoad: "You went too fast around the curve!",
                cutout:  "Motor cutout — you were going too fast too long!",
                tooSlow: "Too slow off the ramp.",
                fellOver:"You stood still too long.",
            }[rider.crashReason] || "";

            if (isGameOver) {
                el.crashReason.textContent = causeText + " You broke your leg — you're done!";
            } else {
                const injuryMsg = game.lives === 2
                    ? "You broke your arm! Shake it off!"
                    : "You broke your wrist! Get back on!";
                el.crashReason.textContent = causeText + " " + injuryMsg;
            }
        }

        // Update the retry button label and behavior based on lives remaining
        if (el.retryBtn) {
            el.retryBtn.textContent = isGameOver ? "RIDE AGAIN" : "GET BACK ON";
        }

        // Show lives remaining as hearts in the stat area
        if (el.livesDisplay) {
            el.livesDisplay.textContent = "❤️".repeat(game.lives) + "🖤".repeat(3 - game.lives);
        }
    ...
    function update(dt) {
        ...
        // Speed control: slider position IS the target speed.
        // Center (0) = stopped, full right (+1) = max forward,
        // full left (-1) = max reverse at the same speed scale.
        // Holding the slider steady at any position holds that speed —
        // no drift up or down. Releasing springs back to center and
        // speed decays naturally toward zero.
        const sliderT = input.sliderT;
        const activeInput = Math.abs(sliderT) > 0.08;

        if (activeInput) {
            // Target speed is directly proportional to slider position.
            // We move toward it quickly so it feels responsive, but with
            // a small smoothing factor so it doesn't snap instantly
            // (which would feel jerky on a real EUC lean).
            const targetSpeed = sliderT * 1.3; // 1.3 = max speedNorm at full throw
            const diff = targetSpeed - game.speedNorm;
            // Approach the target at a rate that feels like real lean physics —
            // faster when the difference is large, gentler near the target
            game.speedNorm += diff * Math.min(1, dt * 3.5);
        } else {
            // Slider released / at center — decay toward a dead stop
        ...
        // Hill physics: the slider still sets your target cruise speed,
        // but gravity acts as a continuous force directly on actual speed —
        // not just a nudge to the target. This means:
        // DOWNHILL (slope < 0): gravity accelerates you forward whether you
        //   want it or not. You MUST push the slider left (brake) to resist
        //   it or the wheel runs away downhill.
        // UPHILL (slope > 0): gravity decelerates you. You must push further
        //   right than normal just to maintain speed; let go = roll backward.
        const slope = hillSlopeAt(game.distance);

        // First apply the slider target (what the rider wants)
        if (activeInput) {
            const targetSpeed = sliderT * 1.3;
            const diff = targetSpeed - game.speedNorm;
            game.speedNorm += diff * Math.min(1, dt * 3.5);
        } else {
            // Slider at center / released — decay toward stop on flat
            // (gravity below will override this on hills)
            const decay = Math.sign(game.speedNorm) * Math.min(Math.abs(game.speedNorm), 0.9 * dt);
            game.speedNorm -= decay;
        }

        // Then apply gravity directly on top — it fights or aids regardless
        // of what the slider says. Downhill gravity is strong enough that
        // you genuinely need to push back against it.
        if (Math.abs(slope) > 0.02) {
            // slope > 0 = uphill = gravity pulls speed DOWN (negative)
            // slope < 0 = downhill = gravity pulls speed UP (positive, runs away)
            game.speedNorm -= slope * HILL_GRAVITY_BRAKE * dt;  // uphill drag
            if (slope < 0) {
                // Extra push on descent — this is what makes the wheel "want
                // to roll downhill" and requires active braking to counter
