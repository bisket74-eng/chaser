...
.eucDodgeBtn {
    ...
    background:transparent; border:none; color:rgba(234,242,248,.6);
    font-family:inherit; padding:0; transition:background .08s,color .08s;
}
.eucDodgeBtn.active { background:rgba(255,255,255,.16); color:#fff; }
...
.eucSliderTrack {
    position:relative; width:160px; height:64px;
    background:rgba(255,255,255,.04); border:1.5px solid rgba(255,255,255,.12);
    border-radius:32px; touch-action:none; user-select:none;
}
.eucSliderMid {
    position:absolute; left:50%; top:50%; transform:translate(-50%,-50%);
    width:2px; height:28px; background:rgba(255,255,255,.18); border-radius:1px; pointer-events:none;
}
.eucSliderLbl {
    position:absolute; top:50%; transform:translateY(-50%);
    font-size:9px; font-weight:700; letter-spacing:.5px; color:rgba(234,242,248,.3); pointer-events:none;
}
...
.eucSliderPuck.fwd  { border-color:rgba(79,209,197,.7); background:radial-gradient(circle at 38% 32%,rgba(79,209,197,.35),rgba(79,209,197,.12)); }
.eucSliderPuck.rev  { border-color:rgba(255,120,80,.7);  background:radial-gradient(circle at 38% 32%,rgba(255,120,80,.35),rgba(255,120,80,.12)); }
...
<div id="eucRoot">
  ...
  <div id="eucBattBar">
    <div id="eucBattLabel">BATTERY</div>
...
window.__eucDriftRunGame = function() {
    ...
    [["eucUp",1],["eucDn",-1]].forEach(([id,dir])=>{
        ...
        on(el,"touchstart",dn,{passive:false}); on(el,"touchend",up,{passive:false}); on(el,"touchcancel",up,{passive:false});
        on(el,"mousedown",dn); on(el,"mouseup",up); on(el,"mouseleave",up);
    });

    // Slider
    const sliderTrack=document.getElementById("eucSliderTrack");
    const puck=document.getElementById("eucPuck");
    const HALF=56, DZ=0.08;
    ...
    function sliderSet(clientX) {
        ...
        const t=Math.max(-1,Math.min(1,(clientX-r.left-r.width/2)/HALF));
        input.sliderT=t; input.leanLeft=t<-DZ; input.leanRight=t>DZ;
        updatePuck(t);
    }
    function sliderRelease() {
        sliderDown=false; input.sliderT=0; input.leanLeft=false; input.leanRight=false;
        puck.style.transition="transform .18s cubic-bezier(.34,1.56,.64,1)";
        updatePuck(0); setTimeout(()=>{if(puck)puck.style.transition="";},220);
    }
    ...
    // ─── Obstacles ──────────────────────────────────────────────────
    const ODEFS={ crate:{w:44,h:44}, cone:{w:26,h:40}, ramp:{w:92,h:46} };
    ...
    function spawnObst(){
        ...
        obstacles.push({id:obstIdSeq++,type,variant:type==="crate"?crateVariant(zone):"cone",x:spX,w:def.w,h:def.h,lane:Math.random()<.5?LANE_TOP:LANE_BOTTOM,triggered:false});
    }
    ...
    function updateBattery(dt){
        const sf=Math.max(0,game.speedNorm/1.3);
        const steep=hillSlope(game.dist)<-.3&&game.speedNorm>.5;
        if(sf>.6||steep){
            const cm=1+(1-game.batt)*2.5;
            game.batt=Math.max(0,game.batt-(0.04+sf*.06)*cm*dt);
        } else if(sf<.3){
            game.batt=Math.min(1,game.batt+.09*dt);
        }
        rider.overheatWarning=game.batt<BATT_WARN&&game.batt>0;
        if(rider.overheatWarning) rider.beepFlash=.3;
        else if(rider.beepFlash>0) rider.beepFlash-=dt;

        if(game.batt<=0&&!rider.crashed&&!rider.cutoutFlying){
            game.batt=0;
            if(game.speedNorm>.3){ rider.cutoutFlying=true; rider.cutoutTimer=.5; }
            else { game.speedNorm=0; game.batt=.05; }
        }
    }
    ...
    function updateRider(dt){
        ...
        input.leanLeft=input.sliderT<-DZ; input.leanRight=input.sliderT>DZ;

        updateBattery(dt);

        rider.laneT+=(rider.targetLaneT-rider.laneT)*Math.min(1,dt*10);
        rider.sitT+=(0-rider.sitT)*Math.min(1,dt*9);
    ...
    function checkCollisions(){
        if(rider.crashed) return;
        const sc=rScale(), h=RIDER_H*sc, w=32*sc;
