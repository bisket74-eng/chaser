    ...
    container.innerHTML = `
        ...
        <div id="eucDriftRoot">
            ...
            <div id="eucDriftHud">
                ...
                <div id="eucLivesHud" style="position:absolute;left:50%;transform:translateX(-50%);top:10px;font-size:16px;letter-spacing:3px;pointer-events:none;">❤️❤️❤️</div>
                <div id="eucSpeedWrap">
...
window.__eucDriftRunGame = function () {
    ...
    const game = {
        ...
        lives: 3,               // 3 strikes before game over
        checkpointZone: 0,      // which zone you've reached (0=city, 1=countryside, 2=track)
        checkpointDistance: 0,  // distance at the start of that zone
    };
    ...
    function setupPad() {
        ...
        function onSliderStart(clientX) {
            ...
            // Read position immediately on touch so it's responsive
    ...
    function updateOverheatState(dt) {
        // Only count as "full throttle" if the slider is pushed to near
        // maximum — 85% or more of full throw. At 35 mph (roughly 50%
        // slider) the overheat timer should not accumulate at all.
        const nearMaxThrottle = input.sliderT > 0.85;
        const onSteepDescent = hillSlopeAt(game.distance) < -0.3 && game.speedNorm > 0.5 && input.sliderT < -0.08;
        const fullThrottle = nearMaxThrottle || onSteepDescent;
    ...
    function respawnRider() {
        // Respawn at the start of the checkpoint zone, speed 0.
        // Distance → scrollX: distance = scrollX * baseScroll * 0.06
        // so scrollX = distance / (baseScroll * 0.06)
    ...
    function endGame(isGameOver) {
        ...
        // Injury message — escalates each time you crash
        if (el.crashReason) {
            if (isGameOver) {
                el.crashReason.textContent = "You broke your leg — you're done!";
            } else {
                const injuryMsg = game.lives === 2
                    ? "You broke your arm! Shake it off!"
                    : "You broke your wrist! Get back on!";
                el.crashReason.textContent = injuryMsg;
            }
        }
    ...
    function update(dt) {
        ...
        // Hill gravity fights against the slider's target speed.
        // Uphill: gravity pulls the effective target DOWN, so you need
        // to push the slider further right just to maintain your speed —
        // let go and you roll backward.
        // Downhill: gravity pushes the effective target UP, so the wheel
        // wants to run away — you need to push left to hold it back.
        const slope = hillSlopeAt(game.distance);
        const hillResistance = slope * 0.55; // how much gravity shifts the effective target
        const adjustedTarget = sliderT > 0.08 || sliderT < -0.08
            ? (sliderT * 1.3) - hillResistance
            : -hillResistance; // even at center, gravity pulls you

        if (activeInput || Math.abs(slope) > 0.05) {
            const diff = adjustedTarget - game.speedNorm;
            game.speedNorm += diff * Math.min(1, dt * 3.5);
        } else {
            // Flat ground, slider centered — decay toward stop
            const decay = Math.sign(game.speedNorm) * Math.min(Math.abs(game.speedNorm), 0.9 * dt);
            game.speedNorm -= decay;
        }
