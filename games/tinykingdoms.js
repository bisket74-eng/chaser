...
function secretGoalText(p, reveal) {
    const goal = secretGoalById(p.secretGoal);
    const me = myPlayer();
    const canSee = reveal || p.isComputer || (me && me.id === p.id);
...
function playerCard(p, index) {
    ...
    return (
        '<div class="tk-player-card ' + (turn ? "turn " : "") + (winner ? "winner " : "") + '">' +
            ...
            '</div>' +
            '<div class="tk-secret ' + (secretLabel ? "bonus-flash" : "") + '">' + escapeHtml(secretGoalText(p, revealSecrets)) +
...
function roundEventHtml(st) {
    ...
    return '<div class="tk-event"><b>' + escapeHtml(ev.icon + ' ' + ev.title) + '</b><span>' + escapeHtml(ev.desc) + '</span></div>';
}
...
function renderTinyKingdoms(skipBotCheck) {
    ...
    if (st.phase === "playing") {
        ...
        actionsHtml = (
            '<div class="tk-actions ' + (canAct ? "my-turn" : "") + '">' +
                actionButton("farm", "🍞", "Farm", currentEventId(st) === "harvest" ? "+4 food" : "+3 food", !canAct, "") +
                actionButton("trade", "🪙", "Trade", currentEventId(st) === "market" ? "+4 coins" : "+3 coins", !canAct, "") +
                actionButton("study", "📚", "Study", currentEventId(st) === "scholar" ? "+3 study" : "+2 study", !canAct, "") +
                actionButton("train", "⚔️", "Train", currentEventId(st) === "training" ? "+3 army" : "+2 army", !canAct, "") +
                actionButton("guard", "🛡️", "Guard", currentEventId(st) === "fortify" ? "+2 shield" : "+1 shield", !canAct, "blue") +
                actionButton("wonder", "✨", "Wonder", "cost " + cost.food + "/" + cost.coins + "/" + cost.science, buildDisabled, "gold") +
                actionButton("raid", "🔥", "Raid", currentEventId(st) === "bandits" ? "raid +4" : "army test", !canAct, "red") +
            '</div>'
        );
    ...
    el.innerHTML = [
        '<style>',
            ...
            '.tk-event{background:#fff6c8;color:#1e4620;border:2px solid #ffd700;border-radius:13px;padding:6px 8px;margin:0 auto 7px;box-shadow:0 3px 8px rgba(0,0,0,.28);line-height:1.12;}',
            '.tk-event b{display:block;font-size:15px;font-weight:900;}',
            '.tk-event span{display:block;font-size:11px;font-weight:900;color:#37603d;margin-top:2px;}',
            ...
            '.tk-actions{margin:8px auto 6px;display:grid;grid-template-columns:repeat(4,1fr);gap:6px;border:3px solid transparent;border-radius:15px;padding:5px;box-sizing:border-box;}',
